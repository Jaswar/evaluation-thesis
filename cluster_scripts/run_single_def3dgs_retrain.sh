...
#SBATCH --output=slurm_output_def3dgs_retrain/slurm-%x-%j.out   # Set name of output log. %j is the Slurm jobId
#SBATCH --error=slurm_output_def3dgs_retrain/slurm-%x-%j.err    # Set name of error log. %j is the Slurm jobId
...
apptainer exec -C --nv \
	...
	--bind /tudelft.net/staff-umbrella/StudentsCVlab/jwarchocki/container_output/best_configs/def3dgs/${CAMERA}/${SCENE}:/opt/models/def3dgs-thesis/arguments/ego_exo/random_configs:rw \
	daic_container.sif \
	/bin/bash -c $"source /opt/miniconda3/etc/profile.d/conda.sh && conda activate def3dgs && cd /opt/models/def3dgs-thesis && python train.py -s ego_exo_data/all_saves/${CAMERA}/${SCENE} -m output/ego_exo/random_search/${CAMERA}/${SCENE}/${REP} --configs arguments/ego_exo/random_configs/${config_name}"
